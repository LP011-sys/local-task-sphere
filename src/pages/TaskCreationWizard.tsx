...
export default function TaskCreationWizard({ onDone }: { onDone?: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      }, 1200); // short confirmation before redirect
    } catch (e: any) {
  ...
  if (redirected) {
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 flex justify-center items-center h-80">
        <div className="bg-white rounded-xl shadow-md p-8 border text-center space-y-3">
          <div className="text-xl font-bold">Task posted!</div>
          <div className="text-muted-foreground">Redirecting to your offers...</div>
        </div>
      </div>
    );
  }
